...
/* globals gDevTools XPCOMUtils gDevToolsBrowser */
...
exports.inspectDOMElement = function (toolbox, selectorArray) {
  return gDevTools.showToolbox(toolbox._target, "inspector").then(tbox => {
    let {
      walker
    } = tbox.getCurrentPanel();
    walker.querySelector(walker.rootNode, selectorArray[0])
      .then(function (result) {
        ...
        while (result.nodeName === "IFRAME" && selectorArray.length > depth) {
          ...
          result = walker.frontForRawNode(
            contentDocument.querySelector(selectorArray[depth]));
          depth += 1;
        }
        tbox.selection.setNodeFront(result);
      });
  });
};
